...
def Add_Tree_GEMM(A, B, conv_param):
    ...
    weights_int_max = 2**(act_bitwidth-1) - 1
    if act_signed:
        act_int_max = 2**(act_bitwidth-1) - 1
        max_add_val = act_int_max*weights_int_max*K
    else:
        act_uint_max = 2**act_bitwidth - 1
        max_add_val = act_uint_max*weights_int_max*K
    
    max_bitwidth = math.floor(math.log(max_add_val, 2)) + 1
    bitwidth_init = max_bitwidth

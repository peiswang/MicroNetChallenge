...
#    load_state_dict(student_model, student_checkpoint['state_dict'])
    ...
    if 'fc_params' in student_checkpoint:
      print('Resuming fc params...')
      fc_params = student_checkpoint['fc_params']
    if 'fc_masks' in student_checkpoint:
      print('Resuming fc masks...')
      fc_masks = student_checkpoint['fc_masks']
    if 'fc_alphas' in student_checkpoint:
      print('Resuming fc alphas...')
      fc_alphas = student_checkpoint['fc_alphas']
  ...
  sp = sparse_quant_param.SparseQuantParam(fc_weights, fc_params, fc_masks, fc_alphas) 
#  sp.update_mask(0)
  with torch.no_grad():
    sp.update_params(args.weight_bitwidth, True)
